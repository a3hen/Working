...
class Connect():
    ...
    def get_ssh_conn(self):
        ...
        for node in self.cluster['node']:
            if local_ip == node['public_ip']:   #判断，如果ip为本机ip，则不加入到列表中
...
class Console:
    def __init__(self):
        self.conn = Connect()
        self.logfilepath = self.conn.cluster['logfilepath'] #此处的logfilepath为yaml文件中logfilepath路径
...
def arg():
    ...
    sub_parser = parser.add_subparsers(help='子命令帮助')
